...
- Whate:
  + Net2Net是以大类型的统称，它是把teacher网络的知识传递给student网络，让student网络从头训练的时候变的更快
  + 这里有两个典型的net2net网络，一个是Net2WiderNet,一个是Net2DeeperNet
  + Net2WiderNet是用宽度相等的model(这里的model的每个隐藏层都有比teacher网络的相应层的输出节点数units要多)代替teacher
  + Net2DeeperNet是用相同的深度代替teacher Model (只是转化方程变了)
# Experiments
