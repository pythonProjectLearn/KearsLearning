...
使用一个CNN，堆叠了一个RNN和一个CTC损失函数，来识别图片上的文字
...
def train(run_name, start_epoch, stop_epoch, img_w):
    ...
    # 卷积池化之后，再把图片分成小方块，然后拼接竖直方向的小方块，使得图片只是被在竖直方向切分，用于识别图片上的字符
    conv_to_rnn_dims = (img_w // (pool_size ** 2), (img_h // (pool_size ** 2)) * conv_filters)
    inner = Reshape(target_shape=conv_to_rnn_dims, name='reshape')(inner)

    # 卷积池化之后，接一个全连接层，输出给RNN
    inner = Dense(time_dense_size, activation='relu', name='dense1')(inner)

    # Two layers of bidirectional GRUs
    # GRU seems to work as well, if not better than LSTM:
    # 正向和反向连接GRUS，go_backwards=True表示反向， add()是网络层的相加，concatenate()表示在竖直方向拼接
...
if __name__ == '__main__':
    run_name = datetime.datetime.now().strftime('%Y:%m:%d:%H:%M:%Seq2seq')
